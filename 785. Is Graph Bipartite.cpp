...
/* Our goal is trying to use two colors to color the graph and see if there are any adjacent nodes having 
the same color. Initialize a color array for each node. Here are three states for colors array:
...
public:
    ...
    bool validColor(vector<vector<int>> &graph, vector<int> &colors, int color, int node) {
        ...
        for (int nbr : graph[node]) {
            if (!validColor(graph, colors, 1-color, nbr)) {
                return false;
            }
        }
        return true;
