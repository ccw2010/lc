...
私有变量，cap_, l_和m_，其中cap_是缓存器的容量大小，l_是保存缓存器内容的列表，m_是哈希表，保存关键值key和缓存器各项的迭代器
之间映射，方便我们以O(1)的时间内找到目标项。
然后再来看get和put如何实现，get相对简单些，我们在m_中查找给定的key，如果存在则将此项移到顶部，并返回value，若不存在返回-1。
对于put，也是现在m_中查找给定的key，如存在就删掉原有项，并在顶部插入新来项，然后判断是否溢出，若溢出则删掉底部项(最不常用项)*/
